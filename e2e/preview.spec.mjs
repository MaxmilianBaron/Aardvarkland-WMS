...
async function assertSourceLoginGeometry(page) {
  ...
  await expect(page.locator('#demo-language-toggle .flag-icon svg')).toBeVisible();
...
test('desktop login and all role shells match the private Server source structure', async ({ browser }) => {
...
test('mobile shell uses the private five-slot tabbar with a More drawer', async ({ browser }) => {
  ...
  await expect(page.locator('.mobile-tabbar > .mobile-tabbar__button')).toHaveCount(5);
...
test('dedicated 411x868 phone frame remains available and runs the same source-parity UI', async ({ browser }) => {
  ...
  await expect(app.locator('[data-role="manager"]')).toHaveClass(/is-active/);
